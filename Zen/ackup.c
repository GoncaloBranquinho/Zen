...
void load_file(FILE *fp, char *extension, WINDOW *text_win, WINDOW *top, WINDOW *bottom, char *filename) { 
	...
	int connected = 0;
	while (flag) {
		char *buffer = calloc(maxX+2,sizeof(char));
		int i = 0, ch;
		if (connected) {
			buffer[0] = ch;
			i = 1;
	...
	while (1) {
		...
		switch (ch) {
			...
			case KEY_UP: {
				...
				if (y > 0) {
					y--;
					if (x > line->prev->text_size) { 
						x = line->prev->text_size;
					}
					line = line->prev;
				}
				break;
			...
			case KEY_BACKSPACE: { // o problema esta quando faço backspace e a linha esta toda cheiaa
				...
				line = back_space(my_win, &head , line, maxX, x, y, "");
				if (x == 0) {
					x = line->text_size;
					if (x == maxX) {
						x = maxX-1;
			...
			case CTRL_Q: {
                ...
                }						// se apagar e a linha de baixo ficar vazia, is_connected fica a 0
				fclose(fp);		
				return;
			}
			case TAB: {
			...
			default: {
				...
				if (mode == EDIT) {
					shift_right(my_win, line, maxX, x, y, ch);
					if (x == maxX-1) {
						line->text_size++;
						y++;
						line = line->next;
